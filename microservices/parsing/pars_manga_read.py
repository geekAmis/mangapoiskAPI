...
def see_next_page_and_this(manga,chapter):
	...
	data = {
		...
		"manga_next": next_page_link.replace('/manga/','').split('/')[0] if 'время выхода главы' not in next_page else manga,
		"chapter_next": next_page_link.split('/')[-1] if 'время выхода главы' not in next_page else chapter
	}
	print(data)
	return data
